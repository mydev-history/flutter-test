...
class CustomerAppointments extends StatefulWidget {
  ...
  const CustomerAppointments({Key? key, required this.customerId}) : super(key: key);

  @override
  _CustomerAppointmentsState createState() => _CustomerAppointmentsState();
}
...
class _CustomerAppointmentsState extends State<CustomerAppointments> {
  ...
  Future<void> _fetchSchedules() async {
    List<Map<String, dynamic>> schedules = await _customerService.getCustomerSchedules(widget.customerId);
    setState(() {
...
Future<void> approveSchedule(String scheduleId) async {
  try {
    final response = await supabase
        .from('Schedule')
        .update({
          'customer_acceptance': true, // ✅ Mark as approved
...
Widget _buildAppointmentCard(Map<String, dynamic> schedule, double screenWidth, double screenHeight) {
  ...
  final scheduledText = schedule['initiated_by'] == 'customer'
      ? "You scheduled an appointment with ${schedule['business_name']} for ${schedule['deal_title']} on ${schedule['scheduled_date']} at ${schedule['scheduled_time']}"
      : "${schedule['business_name']} scheduled ${schedule['deal_title']} on ${schedule['scheduled_date']} at ${schedule['scheduled_time']}";
